...
public class KinectAvatar : MonoBehaviour
{
    ...
    private bool ModelFlag = false; //モデルの表示,非表示のフラグ
    private bool TrackFlag = false; //トラッキングされている人が存在するかのフラグ

    // Use this for initialization
    void Start()
    {
        //変数へモデルのGameObjectを取得
    ...
    {
        //表示、非表示の切り替え
        if (TrackFlag == false && ModelFlag == true)
        {
            //トラッキングされなくなったら非表示へ
        ...
        //BodySouceManagerからデータの取得,エラーチェック
        if (_BodySourceManager == null)
        {
            Debug.Log("_BodySourceManager == null");
            return;
        }
        ...
        var comp = Quaternion.FromToRotation(
            new Vector3(floorPlane.X, floorPlane.Y, floorPlane.Z), Vector3.up);  //compはクウォ―タニオン

        // 関節の回転を取得する
        var joints = body.JointOrientations;　　//IReadOnly Dictionary<JointType, JointOrientation>
        ...
        //床の傾きを考慮しつつそれぞれの関節の角度をクォーテーション型に変換して取得
        // ミラーモード　ToMirrir()で符号反転させている　VectorExtensions.csを作りVector4の拡張としている
        if (IsMirror)
        {
        ...
        // 関節の回転の計算　Kinectのモデルとウニティちゃんのモデルの標準状態の違いをcomp2をかけることで補正
        ...
        Ref.transform.position = new Vector3(-pos.X + 20, pos.Y, -pos.Z);
